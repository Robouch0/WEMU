...
The `core/` sub-project uses clang-format. The configuration is in `core/.clang-format` (LLVM-based style).
...
Format all source files in `core/`:

```bash
find core/src -name '*.cpp' -o -name '*.hpp' | xargs clang-format -i
```
