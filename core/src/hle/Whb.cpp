...
static void hle_WHBLogPrintf(Core::Interpreter &cpu)
{
    ...
    for (int i = 0; fmt[i] && N <= 6; i++) {
        ...
        switch (fmt[i]) {
            case 'd':
            case 'i':
                fprintf(stderr, "%d", (int) args[N++]);
                break;
            case 'u':
                fprintf(stderr, "%u", args[N++]);
                break;
            case 'x':
            case 'X':
                fprintf(stderr, "%X", args[N++]);
                break;
            case 's': {
