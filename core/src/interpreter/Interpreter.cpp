...
void Core::Interpreter::run()
{
    ...
    for (Elf32_Off offset = 0; offset < textSection.header.sh_size; offset += 4) {
        ...
        try {
            std::cout << instructionIDToString(findInstructionID(encodedInstruction)) << std::endl;
...
InstructionID Core::Interpreter::findInstructionID(const EncodedInstruction &encodedInstr)
{
    const auto &candidates = m_instructionMap[encodedInstr.opcd];

    for (const auto &[id, fields] : candidates) {
        const bool found = std::ranges::all_of(fields.begin(), fields.end(), [&](const auto &p) {
            const auto &[field, value] = p;
            switch (field) {
                case Field::F_XO10: return encodedInstr.xo10 == value;
                case Field::F_XO9:  return encodedInstr.xo9  == value;
    ...
    throw Core::InterpreterException("No instruction found with this fields.");
}
