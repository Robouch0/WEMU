...
{
    if (!eval_bo(cpu, instr)) return;

    // BD field: bits [15:2], sign-extended, represents byte offset / 4
    const std::int32_t bd = static_cast<std::int32_t>(
        static_cast<std::int16_t>(instr.raw & 0xFFFC));  // sign-extend 16-bit
...
void BCLR(Core::Interpreter &cpu, const EncodedInstruction &instr)
{
    ...
    if (!eval_bo(cpu, instr)) {
        if (instr.lk) cpu.m_lr = ret; // restore if not taken
        return;
    }
