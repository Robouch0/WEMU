...
{
    m_bin.m_memory = Core::Memory(0x40000000);
    loadHeader();
    loadSections();
    loadSymbols();
}
...
{
    const std::uint32_t align = section.raw.header.sh_addralign > 0 ? section.raw.header.sh_addralign - 1 : 0;
    if (section.raw.header.sh_flags & SHF_EXECINSTR) {
        section.meta.virtAddress = (codeAddressRange.second + align) & ~align;
        codeAddressRange.second = section.meta.virtAddress + section.meta.size;
    } else {
        section.meta.virtAddress = (dataAddressRange.second + align) & ~align;
        dataAddressRange.second = section.meta.virtAddress + section.meta.size;
    }
}
...
// Plan step 1: copy sections data into the memory arena
void Core::Loader::loadSectionsInMemory()
{
    for (auto &section : m_bin.sections) {
        ...
        if (section.meta.type == SHT_NOBITS) {
            ...
            if (ptr) {
                std::memset(reinterpret_cast<void *>(ptr), 0, section.meta.size);
            }
        } else {
            const std::size_t ptr = m_bin.m_memory.allocate(section.meta.virtAddress, section.meta.size);
            if (ptr) {
                std::memcpy(reinterpret_cast<void *>(ptr), section.raw.data.data(), section.raw.data.size());
            }
        }
    }
}
